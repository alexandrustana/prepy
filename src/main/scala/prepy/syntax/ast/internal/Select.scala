...
private[syntax] trait Select {
  ...
  def select[T <: Product](implicit inst: Domain[T], formatter: Formatter = IdentityFormatter): Select.`selectT`[T] =
...
object Select extends Where {
  private[syntax] case class `selectT`[T <: Product](fields: List[Symbol], formatter: Formatter) extends Query {
    ...
    def from[K](implicit typeable: Typeable[K]): Select.`fromT`[T] =
